...
private let before = [
  DisplaySpaceSnapshot(
    identifier: "A", currentSpaceID: 2,
    spaces: [desktop(1), desktop(2), desktop(90, fullscreen: true)])
]
private func after(_ spaces: [ManagedSpaceSnapshot], current: UInt64 = 2, display: String = "A")
  -> [DisplaySpaceSnapshot]
{
...
@Test func creationIsVerifiedOnlyForTheExactNewOrdinaryDesktop() {
  ...
  #expect(
    SpaceTopology.confirmCreation(id: 5, on: "A", before: before, after: appended) == .verified)
  let inserted = after([desktop(1), desktop(2), desktop(5), desktop(90, fullscreen: true)])
  #expect(
    SpaceTopology.confirmCreation(id: 5, on: "A", before: before, after: inserted) == .verified)
}
...
@Test(arguments: [
  ...
  (
    UInt64(5), after(before[0].spaces + [ManagedSpaceSnapshot(id: 5, isFullscreen: false)]),
    "not an ordinary Desktop"
  ),
  (UInt64(5), after(before[0].spaces + [desktop(5)], current: 1), "active Desktop changed"),
  (
    UInt64(5), after([desktop(2), desktop(1), desktop(90, fullscreen: true), desktop(5)]),
    "changed order"
  ),
  ...
  (
    UInt64(5), before + after([desktop(5)], current: 5, display: "B"),
    "display configuration changed"
  ),
])
func creationIsRejectedForEveryOtherTopologyChange(
  id: UInt64, topology: [DisplaySpaceSnapshot], reason: String
) {
  guard
    case .rejected(let message) = SpaceTopology.confirmCreation(
      id: id, on: "A", before: before, after: topology)
  else {
...
@Test func creationOnAnotherDisplayIsRejected() {
  let two =
    before + [DisplaySpaceSnapshot(identifier: "B", currentSpaceID: 10, spaces: [desktop(10)])]
  let elsewhere = [
    two[0],
    DisplaySpaceSnapshot(identifier: "B", currentSpaceID: 10, spaces: [desktop(10), desktop(5)]),
  ]
  #expect(
    SpaceTopology.confirmCreation(id: 5, on: "A", before: two, after: elsewhere)
      == .rejected("The Desktop was created on another display"))
}
...
@Test func dockRegistrationPropagatesReadFailures() {
  ...
  #expect(throws: Moved.self) {
    try DockRegistration.observe(
      expectedCount: 4, read: { throw Moved() }, now: { clock.time }, pause: { clock.time += 0.02 })
  }
}
