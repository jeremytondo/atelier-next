...
/// Every private SkyLight and HIServices symbol the providers use, resolved once
...
/// of the providers keep working without them.
final class PrivateAPI {
  ...
  init() throws {
    ...
    else { throw ProviderError("Could not open SkyLight.framework") }
    guard
      ...
      dlclose(skyLight)
      throw ProviderError("Could not open HIServices.framework")
    }
