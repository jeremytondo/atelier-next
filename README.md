...
`mise run dev` builds and opens the development app, which appears in the menu bar. It needs the Accessibility permission, and without it opens its setup window, which says so and has the way to the right settings pane; **Setup…** in its menu opens that window again. The build is signed with your Apple Development certificate when you have one, so the permission survives rebuilds.
