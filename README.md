...
`mise run dev` builds and opens the development app, which appears in the menu bar. It needs the Accessibility permission; its menu says so and opens the right settings pane. The build is signed with your Apple Development certificate when you have one, so the permission survives rebuilds.
