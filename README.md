...
Use disposable Desktops and saved windows for Desktop switching, create, reorder, and delete; Groups, the group toggle, exact window selection, member reordering, and native Fill; Group presets from a shortcut and the picker, with an app hidden, minimized, and open on another Desktop; the held-modifier overlay with and without Shift, including waiting slots; and Quick Apps. Verify reload during a Space operation, that Groups and their order return after Reload Config and after quitting and reopening Hammerspoon 2, the Accessibility and notification prompts, and `atelier doctor` on a healthy and a broken install. Desktop creation uses macOS's window-management bridge without opening Mission Control and supports one display; reorder and delete still use Mission Control. HS2 remains experimental; upgrades require explicit behaviour verification.
