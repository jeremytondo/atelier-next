...
Groups survive Reload Config and quitting Hammerspoon 2: their identities are saved to `~/Library/Application Support/Atelier/groups.json` and checked against the live windows on the next start, and deleting that file is safe. Failed Fills are retried after a reload. A restart or logout gives every window and Desktop a new identity, so Groups do not survive it.
