...
- **The providers** are one native binary, `atelier-providers`, started by the API as a child of Hammerspoon 2 from inside `Atelier.app`. It inherits the Accessibility grant you give Hammerspoon 2 and hosts one module per gap. When upstream gains an ability, the API function switches to `hs.*` and the provider goes away.
- **The companion** is `Atelier.app`, a background app with no window that owns Atelier's app identity so Spotlight can offer its actions. Each action posts one request to the running session, which answers it through `atelier.dispatch`; the companion never launches Hammerspoon 2, resumes a stopped Atelier, or keeps state, and with Hammerspoon 2 quit or Atelier stopped the action ends silently.
...
- `atelier.dispatch({version: 1, action: "reload-config"})`: the companion's entry point, synchronous. It answers `{version, ok, result}` or `{version, ok: false, error}` and accepts only the listed actions; `reload-config` is the same operation as the shortcut. While running, the session serves the same function over HTTP on port 47820 and writes the port and a per-session secret to `~/Library/Application Support/Atelier/companion.json`, which the companion reads and sends as a bearer token; the file goes away when the session stops. `curl -H "Authorization: Bearer $(jq -r .secret ~/Library/Application\ Support/Atelier/companion.json)" --json '{"version":1,"action":"reload-config"}' http://127.0.0.1:47820/dispatch` sends what the companion sends. A refusal is a Console line; a request without the secret is answered 401 and not logged; a stopped session answers nothing. The port is fixed, and a port already taken shows in the Hammerspoon 2 Console and in `atelier doctor`.
