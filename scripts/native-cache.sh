#!/usr/bin/env bash
# Transfer only the compact unsigned native output, never DerivedData. An
...
case "$operation" in
  pack)
    "$root/scripts/build-native.sh" --locked --verify
  ...
  unpack)
    ...
    if ! tar -tzf "$archive" > "$stage/entries" ||
        ...
        ! receipt_valid "$stage/$kind" "$stage/$kind.json" "$("$root/scripts/native-inputs.sh")"; then
