...
# The tap references upstream's signed release ZIP, so casks wait for a pin
# with a release. GitHub releases still happen so the workflow stays exercised.
casks="$temporary/casks"
"$root/scripts/casks.sh" "$casks" "$manifest" > /dev/null
remote=''
if [[ -f $casks/hammerspoon2.rb ]]; then
...
if [[ -n $remote ]]; then
  git clone --quiet --depth 1 "$remote" "$temporary/tap"
  ...
  echo 'Casks not pushed: hammerspoon2.json has no upstream release yet.'
fi
