...
const modifiers = {cmd:"cmd", command:"cmd", option:"alt", opt:"alt", alt:"alt", ctrl:"ctrl", control:"ctrl", shift:"shift"};
const keys = {grave:"`", minus:"-", equal:"=", "left-bracket":"[", "right-bracket":"]", comma:",", period:".", slash:"/", semicolon:";", quote:"'", backslash:"\\", enter:"return", esc:"escape"};
function shortcut(text) {
  if (typeof text !== "string") throw new Error("A shortcut must be a string");
  const parts = text.toLowerCase().split("-"), mods = [];
...
function defaults() {
  return {spaces:true, groups:true, overlay:true, overlayModifiers:"cmd-option", launchAtLogin:null, bindings:defaultBindings(),
...
function normalize(options = {}) {
  ...
  if (config.launchAtLogin !== null && typeof config.launchAtLogin !== "boolean") throw new Error("launchAtLogin must be true, false, or null");
  if (!Array.isArray(config.quickApps) || config.quickApps.length > 50) throw new Error("quickApps must be an array of at most 50 entries");
  const used = new Map();
  const claim = (text, name) => {
    const parsed = shortcut(text);
    if (used.has(parsed.identity)) throw new Error(name + " conflicts with " + used.get(parsed.identity));
  ...
  for (const [name, text] of Object.entries(config.bindings)) {
    ...
    if (text === "none" || (space && !config.spaces) || (!space && name !== "reload-config" && !config.groups)) continue;
    config.shortcuts.push({name, space, ...claim(text, name)});
  }
  config.quickApps = config.quickApps.map((entry, index) => {
    if (!entry || typeof entry.app !== "string" || !entry.app.trim()) throw new Error("Quick App " + (index + 1) + " needs an app");
    for (const name of Object.keys(entry)) if (!["app", "shortcut", "size"].includes(name)) throw new Error("Unknown Quick App option: " + name);
    if (entry.size && (Object.keys(entry.size).some(k => !["width", "height"].includes(k)) ||
        ![entry.size.width, entry.size.height].every(v => Number.isFinite(v) && v > 0))) throw new Error("Invalid size for " + entry.app);
