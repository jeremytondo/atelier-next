...
function fail(message) { globalThis.atelierSelfTestError = String(message); globalThis.atelierSelfTestDone = true; }
try {
  if (typeof hs.hotkey.create !== "function" || typeof hs.ax.applicationElement !== "function" || typeof hs.canvas.create !== "function") throw new Error("Required HS2 APIs are missing");
  if (atelier.defaults().bindings["desktop-1"] !== "option-1") throw new Error("Bundled defaults were not loaded");
  ...
  globalThis.atelierSelfTestTimer = hs.timer.doAfter(0.01, () => {
    try {
      let output = "";
      globalThis.atelierSelfTestTask = hs.task.create("/usr/bin/printf", ["atelier-hs2-probe"], (code) => {
        // Completion and stream callbacks may arrive in either order.
        globalThis.atelierSelfTestCompletion = hs.timer.doAfter(0.1, () => {
          if (code !== 0 || output !== "atelier-hs2-probe") fail("HS2 task output failed: " + code + ": " + output);
          else exerciseDefaults();
        });
      }, null, (channel, text) => { if (channel === "stdout") output += text; });
...
async function exerciseDefaults() {
  try {
    ...
    atelierProbe.stop();
    for (let n = 0; atelierProbe.helperRunning() && n < 100; n++) await atelierProbeTimers.sleep(0.02);
    if (atelierProbe.helperRunning()) throw new Error("Probe helper did not stop");
    atelierProbeTimers.stop(); globalThis.atelierSelfTestDone = true;
  } catch (error) {
