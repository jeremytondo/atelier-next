...
You can omit `atelier.start()` entirely and use only your own HS2 automations. The Atelier object is supplied by the application bootstrap; `hs` remains available directly. Reload disposes the entire old HS2 context, including custom hotkeys and tasks. Pause/Resume Atelier Defaults only affects objects owned by `atelier`. To apply changed options, reload or stop before calling `atelier.start(newOptions)`.
...
A toggle launches/reopens, unhides, centers, and focuses the configured app on the originating display. Toggling while it is frontmost hides the app and restores the remembered exact window if it still exists on the original active Desktop. Quick Apps stay excluded from Groups. All Desktops assignment can persist, and its Dock fallback may show a menu briefly. Undo the assignment through **Dock → Options → Assign To → None**. Removing a Quick App or quitting does not reverse that macOS setting. Fullscreen overlays and permanent always-on-top behavior are unsupported.
