...
private let commands: [String: HelperProtocol.Handler] = [
  "double": HelperProtocol.handler { (request: Echo) in Reply(doubled: request.number * 2) },
  "fail": HelperProtocol.handler { (_: NoArguments) in throw EngineError("expected failure") },
]

private func respond(_ line: String) -> [String: Any] {
  let text = HelperProtocol.respond(to: line, using: commands)
...
@Test func malformedAndOversizedLinesAreRefusedWithoutAnID() {
  let oversized =
    ...
    + String(
      repeating: "x", count: HelperProtocol.maximumRequestBytes) + "\"}"
...
@Test func responsesAreOneLineWithSortedKeys() {
  let text = HelperProtocol.respond(to: #"{"id":8,"command":"double","number":1}"#, using: commands)
