...
@Test func canonicalConfiguration() throws {
  ...
  let cases: [([String: String], String)] = [
    ...
    (
      ["ATELIER_CONFIG_DIR": "/private/isolated", "XDG_CONFIG_HOME": "/private/xdg"],
      "/private/isolated/init.js"
    ),
  ]
  for (environment, expected) in cases {
    #expect(
      try AtelierConfiguration.location(environment: environment, home: home).path == expected)
  }
...
@Test func seedingPreservesEveryExistingConfiguration() throws {
  ...
  try Data("invalid legacy TOML".utf8).write(
    to: config.deletingLastPathComponent().appendingPathComponent("config.toml"))
  try AtelierConfiguration.seed(at: config, from: template)
  ...
  for completed in [false, true] {
    for running in [false, true] {
      for suppressed in [false, true] {
        ...
        if !completed {
          #expect(startup.phase == .onboarding)
          startup.completeOnboarding()
        }
        if running && !suppressed {
          #expect(startup.phase == .conflict)
          startup.resolveConflict(continueAnyway: true)
        }
