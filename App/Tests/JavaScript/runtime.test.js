...
test("groups retain inactive membership, distinguish PID reuse, and exclude fullscreen", () => {
  ...
  const snap = {
    focused: 2,
    targetDisplay: "D",
    displays: [{id: "D", current: "1", spaces: [{id: "1"}, {id: "2", fullscreen: true}]}],
    windows: [1, 2].map((id) => ({id, pid: 10, space: "1"})),
  };
...
test("helper accepts fragmented replies and rejects pending work on malformed output", async () => {
  ...
  let failure;
  const bridge = new Bridge(hs, "/helper", (error) => {
    failure = error;
  });
  await bridge.start();
  state.replies = false;
  ...
  bridge.receive(line.slice(0, 8));
  bridge.receive(line.slice(8));
...
test("Quick Apps keep absolute app references and exclude those apps from Groups", async () => {
  ...
  assert.equal(request.expectedBundleID, "app." + reference);
  assert.deepEqual(request.size, {width: 700, height: 500});
  app.stop();
});
...
test("pause and defaults failures preserve independent HS2 scripts", async () => {
  for (const failure of [null, "options", "permission", "shortcut", "helper"]) {
    const {hs, state} = fakeHS();
    const independent = hs.hotkey.create(["ctrl"], "z", () => {});
    independent.enable();
