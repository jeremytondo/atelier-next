...
function fakeHS() {
  ...
  const hs = {
    ...
    task: {
      create(path, args, ended, _, output) {
        const task = {
          path,
          args,
          isRunning: false,
          ...
          sendInput(line) {
            ...
            else if (request.command === "quickResolve")
              result = {bundleID: "app." + request.app, name: request.app};
    ...
    hotkey: {
      assignable: () => true,
      getHotkeys: () => keys.filter((k) => k.enabled),
      create(mods, name, callback) {
        const key = {
          mods,
          key: name,
          callback,
    ...
    application: {fromPID: () => null},
    window: {focusedWindow: () => null},
    ax: {
      applicationElement: () => null,
      addWatcher: (...args) => watched.push(args),
      removeWatcher: (...args) => removedWatchers.push(args),
    },
    notify: {show: () => {}},
    reload: () => {
      state.reloaded = true;
