...
struct AtelierCompetitor {
  ...
  static func snapshot() -> [AtelierCompetitor] {
    [("Hammerspoon", "org.hammerspoon.Hammerspoon"), ("Hammerspoon 2", "net.tenshu.Hammerspoon-2")]
      .map { name, id in
        AtelierCompetitor(
          name: name,
...
final class AtelierLaunch {
  ...
  func begin() {
    ...
    startup.begin(
      completed: SettingsManager.shared.hasCompletedOnboarding,
  ...
  private func advance() {
    ...
    case .conflict:
      ...
      alert.informativeText =
        "\(competitors.filter(\.running).map(\.name).joined(separator: " and ")) may register the same shortcuts or change windows while Atelier runs. When switching fully to Atelier, disable its startup/login behavior and consider uninstalling it."
      ...
      if proceed {
        AtelierPreferences.shared.set(
          alert.suppressionButton?.state == .on, for: "suppressCoexistenceWarning")
      }
