...
final class SettingsManager: SettingsManagerProtocol {
  ...
  var configLocation: URL {
    get {
      (try? AtelierHost.configurationURL())
        ?? AtelierHost.resources.appendingPathComponent("unavailable-config.js")
    }
    set {
      // HS2 cannot redirect Atelier's canonical configuration.
    }
  }
...
final class AtelierPreferences {
  ...
  init() {
    if ProcessInfo.processInfo.environment["ATELIER_CONFIG_DIR"] != nil {
      file = try? AtelierHost.configurationURL().deletingLastPathComponent().appendingPathComponent(
        ".atelier-ui.json")
    } else {
      file = nil
    }
    values =
      file.flatMap { try? Data(contentsOf: $0) }.flatMap {
        try? JSONDecoder().decode([String: Bool].self, from: $0)
      } ?? [:]
  }
  ...
  func set(_ value: Bool, for key: String) {
    if ProcessInfo.processInfo.environment["ATELIER_CONFIG_DIR"] != nil {
      ...
      UserDefaults.standard.set(value, forKey: "Atelier." + key)
    }
  }
}
