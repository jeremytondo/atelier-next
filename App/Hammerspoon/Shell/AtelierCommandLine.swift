...
enum AtelierCommandLine {
  static func run() {
    ...
    if probe || bootstrap || lifecycle {
      guard let directory = ProcessInfo.processInfo.environment["ATELIER_CONFIG_DIR"],
        directory.hasPrefix("/")
      else {
      ...
      do {
        ...
        while Date() < deadline
          && engine.eval("globalThis.atelierSelfTestDone === true") as? Bool != true
        {
          RunLoop.current.run(until: Date().addingTimeInterval(0.02))
        }
        let success =
          engine.eval("globalThis.atelierSelfTestDone === true && !globalThis.atelierSelfTestError")
          as? Bool == true
        let detail =
          engine.eval("globalThis.atelierSelfTestError || 'HS2 runtime probe timed out'") as? String
          ?? "Runtime probe failed"
        engine.shutdown()
        guard success else { fail(detail) }
        print("HS2 runtime self-test passed")
        exit(0)
      } catch { fail(error.localizedDescription) }
    }
    if let existing = NSRunningApplication.runningApplications(
      withBundleIdentifier: Bundle.main.bundleIdentifier ?? "com.elevenideas.Atelier"
    )
