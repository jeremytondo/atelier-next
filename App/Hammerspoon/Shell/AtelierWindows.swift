...
final class AtelierWindows {
  ...
  func showWelcome() {
    if let welcome {
      ...
      welcome = show("Welcome to Atelier", view: AtelierWelcomeView())
    }
  }
  func closeWelcome() {
    welcome?.close()
  ...
  func showPermissions() {
    if let permissions {
      permissions.makeKeyAndOrderFront(nil)
      NSApp.activate(ignoringOtherApps: true)
    } else {
      permissions = show("Atelier Permissions", view: AtelierPermissionsView())
    }
  }
}
...
struct AtelierWelcomeView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 16) {
      ...
      Text(
        "Atelier adds keyboard controls for Desktops, Groups, and Quick Apps. Customize its behavior in your JavaScript configuration."
      )
      Text((try? AtelierHost.configurationURL().path) ?? "Configuration path unavailable").font(
        .callout.monospaced()
      ).textSelection(.enabled)
      Text(
        "Accessibility lets Atelier control windows and shortcuts. Your configuration will start after you grant access, or you can continue without it."
      )
      ForEach(launch.competitors.filter { $0.installed && !$0.running }, id: \.name) { competitor in
        Text(
          "\(competitor.name) is installed but stopped. When switching to Atelier, disable its startup/login behavior and consider uninstalling it."
        ).font(.callout)
      }
      HStack {
        ...
        if launch.accessibility {
          ...
          Button("Grant Accessibility") { PermissionsManager.shared.request(.accessibility) }
...
struct AtelierPermissionsView: View {
  ...
  private func refresh() {
    states = Dictionary(
      uniqueKeysWithValues: PermissionsType.allCases.map {
        ($0, PermissionsManager.shared.state($0))
      })
  }
  var body: some View {
    VStack(alignment: .leading, spacing: 16) {
      ...
      Text(
        "Atelier Defaults need Accessibility. Other permissions depend on the HS2 scripts you choose to run."
      )
      ForEach(PermissionsType.allCases, id: \.rawValue) { permission in
        HStack {
          ...
          Button("Open Settings") {
            AtelierHost.open(permission.settingsURL, action: "Open permission settings")
          }
        }
      }
      Button("Check Accessibility and Reload Config") {
        refresh()
        if PermissionsManager.shared.check(.accessibility) {
          try? ManagerManager.shared.reload()
        } else {
          PermissionsManager.shared.request(.accessibility)
        }
      }
    }.padding(24).frame(width: 600).fixedSize(horizontal: false, vertical: true).onAppear {
      refresh()
    }
  }
}
