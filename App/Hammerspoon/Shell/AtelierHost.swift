...
enum AtelierHost {
  ...
  static func configurationURL() throws -> URL {
    try AtelierConfiguration.location(
  ...
  static var version: String {
    ...
    return
      "Atelier \(Bundle.main.object(forInfoDictionaryKey: "AtelierBuildVersion") as? String ?? "local") (HS2 \(pin?["version"] as? String ?? "unknown"))"
  }
  static func prepare() throws {
    try AtelierConfiguration.seed(
      at: configurationURL(), from: resources.appendingPathComponent("DefaultConfig/init.js"))
  }
  static func install(in engine: any JSEngineProtocol, generation: UUID? = nil) throws {
    var engine = engine
    engine["atelierHost"] = [
      "helper": Bundle.main.bundleURL.appendingPathComponent("Contents/Helpers/atelier-engine")
        .path,
    ...
    let encoded = String(
      data: try JSONEncoder().encode(resources.appendingPathComponent("Atelier/index.js").path),
      encoding: .utf8)!
    engine.eval("globalThis.atelier = require(\(encoded)).create(hs, atelierHost);")
    guard
      engine.eval("typeof atelier === 'object' && typeof atelier.start === 'function'") as? Bool
        == true
    else {
      throw NSError(
        domain: "Atelier.Runtime", code: 1,
        userInfo: [
          NSLocalizedDescriptionKey:
            "The bundled Atelier runtime could not load. Reinstall Atelier; your configuration will be preserved."
        ])
    }
  }
  ...
  static func open(_ url: URL, action: String) {
    if !NSWorkspace.shared.open(url) {
      presentError(
        NSError(
          domain: "Atelier.Open", code: 1,
          userInfo: [
            NSLocalizedDescriptionKey:
              "macOS could not open \(url.path.isEmpty ? url.absoluteString : url.path)"
          ]), action: action)
    }
  }
  static func openConfiguration() {
    do {
      try prepare()
      open(try configurationURL(), action: "Open Configuration")
  ...
  static func openReference() {
    open(resources.appendingPathComponent("Configuration.md"), action: "Configuration Reference")
  }
  static func openReleases() {
    open(
      URL(string: "https://github.com/jeremytondo/atelier-next/releases")!, action: "Open releases")
  }
  static func diagnostics() -> [String: Any] {
    ...
    var result: [String: Any] = [
      ...
      "contextAvailable": manager.engine.hasContext(),
      "configurationLoaded": status.configurationLoaded,
    ...
    if manager.engine.hasContext(),
      let json = manager.engine.eval("JSON.stringify(globalThis.atelier && atelier.status())")
        as? String,
