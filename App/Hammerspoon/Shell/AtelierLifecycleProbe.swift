...
enum AtelierLifecycleProbe {
    static func require(_ value: @autoclosure () -> Bool, _ message: String) throws {
        if !value() { throw NSError(domain: "Atelier.Probe", code: 1, userInfo: [NSLocalizedDescriptionKey: message]) }
    }
    ...
    static func residentKB() throws -> Int {
        ...
        task.waitUntilExit()
        guard task.terminationStatus == 0, let value = Int(text.trimmingCharacters(in: .whitespacesAndNewlines)) else {
            throw CocoaError(.coderReadCorrupt)
        }
        return value
    }
    static func run() throws {
        let manager = ManagerManager.shared, engine = manager.engine
        let file = try AtelierHost.configurationURL(), directory = file.deletingLastPathComponent()
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        // Never replace an existing config, even when a probe is invoked by hand.
        try require(!FileManager.default.fileExists(atPath: file.path), "Lifecycle probe requires an empty private configuration directory")
        AtelierHost.probeHelper = true
        func write(_ source: String) throws { try Data(source.utf8).write(to: file, options: .atomic) }
        try Data("module.exports = {value: 42};".utf8).write(to: directory.appendingPathComponent("relative.js"))
        ...
        try Data(#"{"name":"Probe","author":"Atelier","version":"1","description":"Private test"}"#.utf8).write(to: spoon.appendingPathComponent("spoon.json"))
        try Data("module.exports = {value: 17};".utf8).write(to: spoon.appendingPathComponent("init.js"))
        let signed = !CommandLine.arguments.contains("--self-test-no-xpc")
        let source = (signed ? "hs.ipc.start();\n" : "") + """
        globalThis.ticks = 0;
        ...
        """
        try write("globalThis.partialTicks = 0; globalThis.partialTimer = hs.timer.doEvery(0.01, () => partialTicks++); throw new Error('expected isolated failure');")
        manager.start()
        // Let callbacks run before another evaluation can clear a stale exception.
        RunLoop.current.run(until: Date().addingTimeInterval(0.1))
        try require(engine.eval("partialTicks > 1") as? Bool == true, "Configuration error stopped an independent timer")
        try write(source + "\nthrow new Error('expected partial configuration failure');")
        try? manager.reload()
        try require(AtelierStatus.shared.configurationError.contains("expected partial"), "Configuration exception was not presented")
        try wait { (engine.eval("ticks > 1 && atelier.state === 'Running'") as? Bool) == true }
        try require(!AtelierStatus.shared.configurationError.isEmpty, "Async defaults status hid the configuration exception")
        try require(engine.eval("relativeValue === 42 && spoonValue === 17") as? Bool == true, "Relative module/Spoon loading failed")
        try require(engine.eval("require('util').inspect({answer:42}).includes('42')") as? Bool == true, "Upgraded Node built-in loading failed")
        ...
        try require(engine.eval("atelier.state === 'Paused'") as? Bool == true, "Pause did not stop defaults")
        manager.resume()
        try wait { engine.eval("atelier.state === 'Running'") as? Bool == true }
        // Native service failures cannot tear down working scripts.
        AtelierHost.presentError(CocoaError(.fileWriteNoPermission), action: "Expected diagnostics failure")
        ...
        engine.eval("globalThis.realHelperRunning = atelier.helperRunning; atelier.helperRunning = () => true;")
        do { try manager.reload(); throw CocoaError(.validationMissingMandatoryProperty) }
        catch { try require(AtelierStatus.shared.configurationError.contains("still stopping"), "Unsafe helper teardown was allowed") }
        ...
        if signed {
            ...
            try FileManager.default.createSymbolicLink(at: cli, withDestinationURL: Bundle.main.bundleURL.appendingPathComponent("Contents/MacOS/hs2"))
            ...
            input.fileHandleForWriting.write(Data("40 + 2\nthrow new Error('expected CLI error')\n'recovered after error'\nconsole.log('expected streamed log')\n".utf8))
            RunLoop.current.run(until: Date().addingTimeInterval(0.3))
            try input.fileHandleForWriting.close()
            try wait { !process.isRunning }
            let text = String(decoding: output.fileHandleForReading.readDataToEndOfFile(), as: UTF8.self)
            try require(process.terminationStatus == 0 && text.contains("42") && text.contains("expected CLI error") && text.contains("recovered after error") && text.contains("expected streamed log"), "Bundled/symlinked CLI IPC failed: \(text)")
            ...
            do {
                let untrusted = try HSIPCSocketConnection(serviceName: HSIPCServer.serviceName,
            ...
            let foreign = Process(), foreignInput = Pipe(), foreignOutput = Pipe()
            ...
            let request = #"{"kind":"evaluate","id":"foreign","code":"globalThis.foreignIPCPeerRan = true","result":"","isError":false,"level":0,"logLevel":""}"# + "\n"
            ...
            let rejected = String(decoding: foreignOutput.fileHandleForReading.readDataToEndOfFile(), as: UTF8.self)
            try require(!rejected.contains("\"kind\":\"reply\"") && engine.eval("typeof foreignIPCPeerRan === 'undefined'") as? Bool == true,
                        "IPC accepted an unsigned/differently signed client")
            for _ in 0..<5 {
                engine.eval("hs.ipc.stop(); hs.ipc.start();")
                try require(engine.eval("hs.ipc.isListening") as? Bool == true, "IPC stop/start did not release its socket")
            }
        }
        try write("globalThis.shouldNotRun = true; {{{ syntax !!!")
        try? manager.reload()
        try require(engine.eval("typeof shouldNotRun === 'undefined'") as? Bool == true, "Syntax error executed code")
        ...
        for index in 0...50 {
            try autoreleasepool {
                ...
                if signed { try require(engine.eval("hs.ipc.isListening") as? Bool == true, "Reload lost IPC") }
            }
        ...
        try wait { JSEngine.shared.id != previousID && engine.eval("typeof ticks !== 'undefined' && ticks > 1 && atelier.state === 'Running'") as? Bool == true }
        try require(manager.shutdown(), "Quit failed to clean up")
        try require(!engine.hasContext(), "Quit retained the context")
        try require(AtelierHost.diagnostics()["runtime"] == nil, "Native fallback diagnostics unavailable")
        let report: [String: Any] = ["version": AtelierHost.version, "reloads": 50, "samples": samples, "passed": true]
        try JSONSerialization.data(withJSONObject: report, options: [.prettyPrinted, .sortedKeys]).write(to: directory.appendingPathComponent("lifecycle.json"), options: .atomic)
        print("Packaged lifecycle probe passed (50 reloads); memory samples: \(directory.appendingPathComponent("lifecycle.json").path)")
    }
}
