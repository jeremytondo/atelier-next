...
final class ManagerManager {
  ...
  func reload() throws {
    ...
    do {
      ...
      guard FileManager.default.changeCurrentDirectoryPath(file.deletingLastPathComponent().path)
      else {
  ...
  func pause() {
    guard !transitioning else { return }
    do { try stopDefaults() } catch {
      AtelierHost.presentError(error, action: "Pause Atelier Defaults")
    }
  }
  ...
  func resume() {
    ...
    let report: @convention(block) (JSValue) -> Void = { error in
      AKError("Atelier defaults: \(error)")
    }
    defaults.invokeMethod("start", withArguments: [])?.invokeMethod(
      "catch", withArguments: [report])
  }
  ...
  private func stopDefaults() throws {
    ...
    while defaults.invokeMethod("helperRunning", withArguments: [])?.toBool() == true {
      guard Date() < deadline else {
        throw NSError(
          domain: "Atelier.Lifecycle", code: 1,
          userInfo: [
