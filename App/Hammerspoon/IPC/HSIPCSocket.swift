...
nonisolated enum HSIPCSocket {
    ...
    throws -> T
  {
    ...
    guard bytes.count <= MemoryLayout.size(ofValue: address.sun_path) else {
      throw error("IPC path is too long")
    }
    ...
    return try unsafe withUnsafePointer(to: &address) {
      try unsafe $0.withMemoryRebound(to: sockaddr.self, capacity: 1) {
        try unsafe body($0, socklen_t(MemoryLayout<sockaddr_un>.size))
      }
    }
  }
  static func configure(_ fd: Int32) {
    ...
    var yes: Int32 = 1
    _ = unsafe setsockopt(
      fd, SOL_SOCKET, SO_NOSIGPIPE, &yes, socklen_t(MemoryLayout.size(ofValue: yes)))
  }
  static func authenticate(_ fd: Int32, requirement: String?) -> Bool {
    ...
    guard unsafe getsockopt(fd, SOL_LOCAL, LOCAL_PEERTOKEN, &token, &length) == 0 else {
      return false
    }
    ...
    guard
      unsafe SecCodeCopyGuestWithAttributes(
        nil, [kSecGuestAttributeAudit: data] as CFDictionary, [], &code) == errSecSuccess,
      let code,
      unsafe SecRequirementCreateWithString(requirement as CFString, [], &rule) == errSecSuccess,
      let rule
    else { return false }
...
nonisolated protocol HSIPCSocketListenerDelegate: AnyObject {
  func listener(
    _ listener: HSIPCSocketListener, shouldAcceptNewConnection connection: HSIPCSocketConnection
  ) -> Bool
}
...
nonisolated final class HSIPCSocketListener: @unchecked Sendable {
  ...
  func resume() throws {
    ...
    guard flock(lock, LOCK_EX | LOCK_NB) == 0 else {
      close(lock)
      throw HSIPCSocket.error("IPC is already running")
    }
    let fd = socket(AF_UNIX, SOCK_STREAM, 0)
    guard fd >= 0 else {
      close(lock)
      throw HSIPCSocket.error("Cannot create IPC socket")
    }
    do {
      ...
      guard try unsafe HSIPCSocket.address(path, { unsafe Darwin.bind(fd, $0, $1) }) == 0,
        Darwin.listen(fd, 16) == 0
      else { throw HSIPCSocket.error("Cannot listen for IPC") }
      let source = DispatchSource.makeReadSource(
        fileDescriptor: fd, queue: DispatchQueue(label: name))
      source.setEventHandler { [weak self] in
        ...
        while true {
          ...
          guard HSIPCSocket.authenticate(peer, requirement: self.requirement) else {
            close(peer)
            continue
          }
          let connection = HSIPCSocketConnection(descriptor: peer)
          if self.delegate?.listener(self, shouldAcceptNewConnection: connection) != true {
...
nonisolated final class HSIPCSocketConnection: NSObject, @unchecked Sendable {
  ...
  convenience init(serviceName: String, requirement: String?) throws {
    ...
    do {
      ...
      if result != 0 {
        ...
        guard unsafe poll(&pollFD, 1, 5000) > 0 else {
          throw HSIPCSocket.error("IPC connection timed out")
        }
      }
      guard HSIPCSocket.authenticate(fd, requirement: requirement) else {
        throw HSIPCSocket.error("IPC peer authentication failed")
      }
  ...
  private func read() {
    ...
    while !closed {
      ...
      if count < 0 {
        if errno != EAGAIN && errno != EINTR { finish() }
        return
      }
      if count == 0 {
        finish()
      ...
      guard incoming.count <= HSIPCSocket.limit else {
        finish()
        return
      }
      while let end = incoming.firstIndex(of: 10) {
        guard
          let message = try? JSONDecoder().decode(
            HSIPCMessage.self, from: incoming.prefix(upTo: end))
        else {
          finish()
          return
        }
  ...
  private func receive(_ message: HSIPCMessage) {
    ...
    case "hello":
      (exportedObject as? HSIPCServerProtocol)?.hello(minLogLevel: message.level) {
        [weak self] result in
        self?.send(HSIPCMessage(kind: "reply", id: message.id, result: result))
      }
    case "evaluate":
      (exportedObject as? HSIPCServerProtocol)?.evaluate(id: message.id, code: message.code) {
        [weak self] result, isError in
        self?.send(HSIPCMessage(kind: "reply", id: message.id, result: result, isError: isError))
      }
    case "log":
      (exportedObject as? HSIPCClientProtocol)?.logEntry(
        level: message.logLevel, message: message.result)
    default: finish()
    }
  }
  fileprivate func request(
    _ message: HSIPCMessage, reply: @escaping (HSIPCMessage?, Error?) -> Void
  ) {
    let box = Reply(callback: reply)
    queue.async {
      guard !self.closed else {
        box.callback(nil, HSIPCSocket.error("IPC connection closed"))
        return
      }
  ...
  private func enqueue(_ message: HSIPCMessage) {
    ...
    guard outgoing.count + data.count + 1 <= HSIPCSocket.limit else {
      finish()
      return
    }
  ...
  private func flush() {
    while !closed && !outgoing.isEmpty {
      let count = unsafe outgoing.withUnsafeBytes {
        unsafe Darwin.send(fd, $0.baseAddress, $0.count, 0)
      }
      if count < 0 {
...
private nonisolated final class HSIPCSocketProxy: NSObject, HSIPCServerProtocol, HSIPCClientProtocol
{
  ...
  func hello(minLogLevel: Int, withReply reply: @escaping (String) -> Void) {
    connection.request(HSIPCMessage(kind: "hello", level: minLogLevel)) {
      [errorHandler] message, error in
  ...
  func evaluate(id: String, code: String, withReply reply: @escaping (String, Bool) -> Void) {
    connection.request(HSIPCMessage(kind: "evaluate", id: id, code: code)) {
      [errorHandler] message, error in
      if let message {
        reply(message.result, message.isError)
      } else if let error {
        errorHandler(error)
      }
    }
