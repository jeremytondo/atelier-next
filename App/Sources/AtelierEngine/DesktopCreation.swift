...
enum DesktopCreation {
  ...
  static func createAndEnter(
    ...
    guard !seams.missionControlVisible() else {
      throw DesktopCreationError(message: "Close Mission Control before creating a Desktop", createdID: nil)
    }
    ...
    var created = before
    let confirmationStarted = seams.now()
    repeat {
      created = seams.topology()
      observation = SpaceTopology.confirmCreation(id: id, on: display, before: before, after: created)
      if observation != .pending { break }
      seams.pause()
    } while seams.now() - confirmationStarted < confirmationTimeout
    switch observation {
    case .verified: break
    ...
    let registration = try DockRegistration.observe(
      ...
      read: {
        ...
        return count
      }, now: seams.now, pause: seams.pause)
  ...
  private static func poll(
    _ seams: DesktopCreationSeams,
    timeout: TimeInterval,
    until condition: ([DisplaySpaceSnapshot]) -> Bool
  ) -> Bool {
    let started = seams.now()
    repeat {
      if condition(seams.topology()) { return true }
      seams.pause()
    } while seams.now() - started < timeout
    return condition(seams.topology())
  }
}
