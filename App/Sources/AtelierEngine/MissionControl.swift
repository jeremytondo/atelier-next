...
final class MissionControl {
  ...
  func activateAdjacentDesktop(
    ...
    throws
  {
    guard seams.isVisible() else { throw EngineError("Mission Control is no longer open") }
    guard let (display, currentIndex) = activeDesktop(on: target, topology: topology) else {
      throw EngineError("No ordinary Desktop is currently active")
    }
    ...
    else { throw EngineError("Could not resolve the adjacent Desktop route") }
    ...
    else { throw EngineError("The active Desktop thumbnail did not remain expanded") }
  }
  ...
  func enterActiveDesktop(on target: TargetDisplay, topology: [DisplaySpaceSnapshot]) throws {
    ...
    else { throw EngineError("Could not resolve the active Desktop thumbnail") }
    guard seams.press(target.displayID, fullIndex) else {
      throw EngineError("The active Desktop rejected selection")
    }
    guard waitForClose(timeout: 3) || close(timeout: 2) else {
      throw EngineError("Mission Control did not close after entering the Desktop")
    }
    guard currentSpaceID(on: target) == active.id else {
      throw EngineError("The active Desktop changed while Mission Control closed")
    }
  }
    ...
    throws
  {
    ...
    else { throw EngineError("Could not resolve the active Desktop thumbnails") }
    ...
    guard seams.drag(sourcePoint, destinationPoint) else {
      throw EngineError("Could not synthesize the Desktop drag")
    }
    guard
      seams.poller.wait(
        3,
    ...
    else { throw EngineError("macOS did not confirm the Desktop reorder") }
    guard ensureExpandedSpacesBar(on: target, topology: seams.topology()) else {
      throw EngineError("The reordered Desktop thumbnail did not remain expanded")
    }
  }
  ...
  func deleteActiveDesktop(on target: TargetDisplay, topology: [DisplaySpaceSnapshot]) throws {
    ...
    let desktops = display.regularDesktops
    guard desktops.count > 1 else { throw EngineError("The final Desktop cannot be deleted") }
    ...
    do {
      ...
      throw EngineError("Could not leave the active Desktop before deletion: \(error.message)")
    }
    ...
    else { throw EngineError("Could not resolve the former active Desktop thumbnail") }
    guard seams.removeDesktop(target.displayID, fullIndex) else {
      throw EngineError("Desktop \(activeIndex + 1) rejected deletion")
    }
    ...
    else { throw EngineError("macOS did not confirm Desktop deletion") }
    guard updated.currentSpaceID == neighbor.id else {
      throw EngineError("The neighboring Desktop did not remain active")
    }
    guard ensureExpandedSpacesBar(on: target, topology: [updated]) else {
      throw EngineError("The neighboring Desktop thumbnail did not remain expanded")
    }
  }
