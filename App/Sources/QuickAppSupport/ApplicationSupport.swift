...
private enum SpaceAssignmentError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .privateAPIFailed(let code):
...
public final class SpaceAssignmentCoordinator {
  ...
  public init() {
    ...
    copyManagedDisplaySpaces =
      skyLightHandle
      .flatMap {
        dlsym($0, "SLSCopyManagedDisplaySpaces") ?? dlsym($0, "CGSCopyManagedDisplaySpaces")
      }
  ...
  private func ordinaryDesktopCount() -> Int {
    guard let connectionID, let copyManagedDisplaySpaces,
      ...
      let displays = value as? [[String: Any]]
    else {
      return 0
    }
  ...
  private func assignThroughDock(target: TargetApplication) async throws {
    ...
    guard
      let dockItem = descendants(of: dockRoot, maximumDepth: 5).first(where: {
    ...
    guard
      let allDesktopsItem = await waitForMenuItem(
        ...
        in: dockRoot,
        timeout: 0.8
      ), boolAXAttribute(allDesktopsItem, kAXEnabledAttribute)
    else {
      dismissMenu()
      throw SpaceAssignmentError.dockUnavailable(
