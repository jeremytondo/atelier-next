...
/// what can be done about it. System type and colors; nothing drawn by hand.
struct SetupView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 16) {
      Section(
        title: "Accessibility", isGood: model.hasAccessibility,
  ...
  private var keys: String {
    let leader =
      model.leaderKey.map { "\($0) opens the leader menu in the corner of the screen." }
      ?? "The leader menu has no key; set one under [leader] in the configuration."
    return
      "\(leader) Hold \(model.windowListModifiers) to see this Desktop's numbered windows. The atelier config show command lists every key."
  }
