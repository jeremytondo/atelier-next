...
final class HS2Bridge {
  ...
  func focusedID() -> UInt32 {
    guard let pid = NSWorkspace.shared.frontmostApplication?.processIdentifier,
      ...
      CFGetTypeID(focused) == AXUIElementGetTypeID() else { return 0 }
  ...
  func frame(_ element: AXUIElement) -> [String: Double]? {
    ...
    guard AXValueGetValue(unsafeDowncast(p, to: AXValue.self), .cgPoint, &point),
      AXValueGetValue(unsafeDowncast(s, to: AXValue.self), .cgSize, &size) else { return nil }
  ...
  private func inventory() -> [[String: Any]] {
    ...
    return descriptions.compactMap { d in
      guard let pid = (d[kCGWindowOwnerPID as String] as? NSNumber)?.int32Value,
        pid != getpid(), let running = NSRunningApplication(processIdentifier: pid), !running.isHidden,
        (d[kCGWindowLayer as String] as? Int) == 0,
        let wid = (d[kCGWindowNumber as String] as? NSNumber)?.uint32Value else { return nil }
      if cache[pid] == nil {
        let app = AXUIElementCreateApplication(pid)
        AXUIElementSetMessagingTimeout(app, 0.35)
      ...
      guard let window = cache[pid]?.first(where: { id($0) == wid }),
        ...
        attribute(window, "AXFullScreen") as? Bool != true else { return nil }
      ...
      guard position.boolValue, size.boolValue, membership.count == 1, let rect = frame(window) else { return nil }
      return ["id": wid, "pid": pid, "space": membership[0], "frame": rect,
  ...
  private func snapshot() -> [String: Any] {
    ...
    return ["trusted": AXIsProcessTrusted(), "pid": getpid(), "focused": focusedID(),
      "targetDisplay": target?.topologyIdentifier ?? "", "missionControl": missionControl.isVisible(),
      "displays": topology.map { d -> [String: Any] in
        ["id": d.identifier, "current": String(d.currentSpaceID),
          "spaces": d.spaces.map { ["id": String($0.id), "fullscreen": $0.isFullscreen] as [String: Any] }]
  ...
  func handle(_ line: String) async {
    ...
    do {
      guard line.utf8.count < 65536,
        ...
        let command = request["command"] as? String else { throw BridgeError(message: "Invalid JSON request") }
      response["id"] = request["id"] ?? NSNull()
      if command == "quickToggle" {
        guard AXIsProcessTrusted() else { throw BridgeError(message: "Accessibility permission required for quick apps") }
  ...
  private func execute(_ command: String, _ request: [String: Any]) throws -> [String: Any] {
    ...
    if command == "membership" {
      guard let wid = request["window"] as? UInt32 else { throw BridgeError(message: "window required") }
      return ["spaces": spaces(wid), "focused": focusedID()]
    }
    guard AXIsProcessTrusted() else { throw BridgeError(message: "Accessibility permission required for the helper's responsible app") }
    if command == "fill" || command == "focus" {
      guard let wid = request["window"] as? UInt32, let pid = request["pid"] as? Int32,
        let window = element(pid: pid, window: wid) else { throw BridgeError(message: "Exact window no longer exists") }
      ...
      if command == "focus" {
        ...
        guard wait(until: { focusedID() == wid }) else { throw BridgeError(message: "Could not verify exact window focus") }
      } else {
        guard focusedID() == wid else { throw BridgeError(message: "Fill target lost focus") }
        _ = try NativeMenuDispatcher.dispatch(identifier: "_zoomFill:", commandName: "Fill", processID: pid, window: window)
    ...
    guard let target = resolver.resolve(in: before),
      let display = before.first(where: { $0.identifier == target.topologyIdentifier }) else { throw BridgeError(message: "No target display") }
    ...
    if command == "send" {
      throw BridgeError(message: "Window movement between Spaces is disabled in this prototype")
    }
    if command == "switch" {
      guard let number = request["number"] as? Int,
        let desktop = SpaceTopology.desktop(number: number, on: target.topologyIdentifier, displays: before) else { return ["noop": true] }
      if desktop.id != display.currentSpaceID {
        let restoreDeadline = Date().addingTimeInterval(0.27)
        guard let global = SpaceTopology.globalDesktopNumber(for: desktop.id, displays: before), global <= 16,
          runtime.postSymbolicHotKey(UInt32(117 + global)) else { throw BridgeError(message: "Native Desktop shortcut unavailable") }
        guard wait(until: { runtime.snapshot().first(where: { $0.identifier == target.topologyIdentifier })?.currentSpaceID == desktop.id }) else { throw BridgeError(message: "Native shortcut sent but destination was not verified") }
        if Date() < restoreDeadline { RunLoop.current.run(until: restoreDeadline) }
      }
      // Allow temporary symbolic registrations to be restored before HS2 reenables its bindings.
      runtime.restoreTemporarilyEnabledHotKeys()
      return snapshot()
    }
    guard ["create", "reorder", "delete"].contains(command) else { throw BridgeError(message: "Unknown command: \(command)") }
    if command == "reorder", ![-1, 1].contains(request["offset"] as? Int ?? 0) {
      throw BridgeError(message: "offset must be -1 or 1")
    }
    if command == "delete" && display.regularDesktops.count <= 1 { throw BridgeError(message: "The final Desktop cannot be deleted") }
    let departingWindows = command == "delete"
      ? inventory().filter { $0["space"] as? String == String(display.currentSpaceID) }.compactMap { $0["id"] as? UInt32 }
      : []
    defer { cleanup() }
    if command == "create" {
      ...
      var result = snapshot(); result["created"] = String(created); return result
    }
    if !missionControl.isVisible() {
      guard runtime.postSymbolicHotKey(32), wait(until: { missionControl.isVisible() }) else { throw BridgeError(message: "Could not open Mission Control") }
    }
    _ = try missionControl.beginKeyboardNavigation(on: target, topology: before).get()
    if command == "reorder" {
      guard let offset = request["offset"] as? Int, abs(offset) == 1 else { throw BridgeError(message: "offset must be -1 or 1") }
      _ = try missionControl.reorderActiveDesktop(offset: offset, on: target, topology: runtime.snapshot()).get()
    } else {
    ...
    if command == "delete" {
      ...
      }) else { throw BridgeError(message: "Desktop deleted, but could not verify all eligible windows survived on another Space") }
      result["migratedWindows"] = departingWindows.map { ["id": $0, "spaces": spaces($0)] as [String: Any] }
    }
    return result
  }
}
...
func runHS2Bridge() throws {
  ...
  DispatchQueue.global().async {
    while let line = readLine() {
      ...
      Task { @MainActor in await bridge.handle(line); done.signal() }
      done.wait()
    }
    DispatchQueue.main.async { bridge.cleanup(); app.terminate(nil) }
  }
