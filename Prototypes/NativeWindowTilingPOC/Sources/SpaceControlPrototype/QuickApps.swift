...
extension HS2Bridge {
  ...
  private func quickWindows(_ app: NSRunningApplication) -> [AXUIElement] {
    ...
    return (attribute(root, kAXWindowsAttribute) as? [AXUIElement] ?? []).filter {
      attribute($0, kAXRoleAttribute) as? String == kAXWindowRole
        && [kAXStandardWindowSubrole, kAXDialogSubrole].contains(attribute($0, kAXSubroleAttribute) as? String ?? "")
  ...
  private func placeQuickWindow(_ window: AXUIElement, on displayID: CGDirectDisplayID,
                                size: [String: Double]?) async throws {
    ...
    try setQuickPosition(window, CGPoint(x: usable.midX - min(width, usable.width) / 2,
                                        y: usable.midY - min(height, usable.height) / 2))
    if size != nil || width > usable.width || height > usable.height {
      var requested = CGSize(width: min(size?["width"] ?? width, usable.width),
    ...
    for _ in 0..<20 {
      ...
      if let settled = frame(window),
        ...
        abs((settled["w"] ?? 0) - w) < 3, abs((settled["h"] ?? 0) - h) < 3 { return }
    }
  ...
  func toggleQuickApp(_ request: [String: Any]) async throws -> [String: Any] {
    ...
    if request["size"] != nil {
      guard let size, let w = size["width"], let h = size["height"],
        w.isFinite, h.isFinite, w > 0, h > 0 else { throw BridgeError(message: "Invalid quick app size") }
    }
    ...
    if let running, !running.isHidden,
      ...
      quickWindows(running).contains(where: { attribute($0, kAXMinimizedAttribute) as? Bool != true }) {
      let appElement = AXUIElementCreateApplication(running.processIdentifier)
      guard running.hide() || AXUIElementSetAttributeValue(appElement, kAXHiddenAttribute as CFString, kCFBooleanTrue) == .success else {
        throw BridgeError(message: "Could not hide \(target.name)")
      }
      let deadline = Date().addingTimeInterval(1)
      while !running.isHidden && Date() < deadline { try await quickPause() }
      guard running.isHidden else { throw BridgeError(message: "Could not verify quick app was hidden") }
      var restored = false
      if let state = quickStates[bundleID], state.pid == running.processIdentifier,
        let previous = state.previous, !previous.isTerminated,
        runtime.snapshot().first(where: { $0.identifier == state.display }).map({ String($0.currentSpaceID) }) == state.space,
    ...
    guard let display = resolver.resolve(in: topology),
      ...
      desktop.regularDesktops.contains(where: { $0.id == desktop.currentSpaceID }) else {
      throw BridgeError(message: "Quick apps require an ordinary Desktop; fullscreen and Split View are unsupported")
    }
    let space = String(desktop.currentSpaceID)
    let frontmost = NSWorkspace.shared.frontmostApplication
    let remembered = quickStates[bundleID]
    let reusePrevious = frontmost?.bundleIdentifier == bundleID && remembered?.display == display.topologyIdentifier && remembered?.space == space
    ...
    func checkDesktop() throws {
      guard runtime.snapshot().first(where: { $0.identifier == display.topologyIdentifier }).map({ String($0.currentSpaceID) }) == space else {
    ...
    if app == nil || quickWindows(app!).isEmpty {
      ...
      app = try await NSWorkspace.shared.openApplication(at: target.url, configuration: configuration)
    }
    ...
    repeat {
      ...
      let remembered = quickStates[bundleID]
      selected = windows.first(where: { remembered?.pid == app.processIdentifier && id($0) == remembered?.window })
        ?? windows.first(where: { id($0) == focusedID() }) ?? windows.first
      if selected != nil { break }
      try await quickPause()
    } while Date() < deadline
    guard let window = selected else {
      throw BridgeError(message: "\(target.name) did not expose a standard window; close fullscreen mode or open its main window")
    }
    try checkDesktop()
    ...
    if !app.unhide() {
      AXUIElementSetAttributeValue(AXUIElementCreateApplication(app.processIdentifier), kAXHiddenAttribute as CFString, kCFBooleanFalse)
    }
    ...
    let assignment = try await quickAssignment.ensureAllDesktops(application: app, window: window, target: target, requiredSpaceIDs: expected)
    try checkDesktop()
    // Keep this identity even if later placement fails, allowing a second press to hide.
    quickStates[bundleID] = QuickAppState(pid: app.processIdentifier, window: id(window),
      previous: previous?.bundleIdentifier == bundleID ? nil : previous, previousWindow: previousWindow,
      display: display.topologyIdentifier, space: space)
    let membershipDeadline = Date().addingTimeInterval(1)
    while !expected.isSubset(of: Set(spaces(id(window)))) && Date() < membershipDeadline { try await quickPause() }
    guard expected.isSubset(of: Set(spaces(id(window)))) else {
      throw BridgeError(message: "Quick app is not available on every Desktop of the captured display")
    }
    AXUIElementSetAttributeValue(window, kAXMainAttribute as CFString, kCFBooleanTrue)
    ...
    try checkDesktop()
    guard focusedID() == id(window) else { throw BridgeError(message: "Could not focus the quick app window") }
    return ["action": "shown", "bundleID": bundleID, "window": id(window), "display": display.topologyIdentifier,
